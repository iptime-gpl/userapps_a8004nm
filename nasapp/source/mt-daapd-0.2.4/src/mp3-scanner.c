/*
 * $Id: mp3-scanner.c,v 1.6 2013/04/19 09:43:58 thki81 Exp $
...
#ifdef EFM_PATCH /* added by thki81 20120216 */
#define SCAN_LOG_PATH "/home/itunes/daapd_scan"
static int log_scan_path(char *path)
{
	FILE *fp;
	fp = fopen(SCAN_LOG_PATH, "w+");
	if (fp)
	{
		fputs(path, fp);
		fclose(fp);
...
static int remove_scan_path(void)
{
	unlink(SCAN_LOG_PATH);
	return 0;
}
#endif // EFM_PATCH
/*
...
#ifdef EFM_PATCH /* added by thki81 20120216 */
			log_scan_path(mp3_path); // leave current filepath.

			MP3FILE *pmp3 = NULL;

			pmp3 = db_find(sb.st_ino);
			if (pmp3)
			{
				if (strcmp(mp3_path, pmp3->path))
				{
					db_delete(sb.st_ino);
...
//debug_Aaron
int i, save_inbytes = inbytesleft;
memcpy(save_buf, inbuf, inbytesleft);
...
//debug_Aaron
        //DPRINTF(E_LOG,L_SCAN,"iconv(to_ces=%s, from_ces=%s), rc=%d, save_inbytes=%d\n", to_ces, from_ces, rc, save_inbytes);
#if 0 
         for (i = 0; i < save_inbytes; i++)
        {
                DPRINTF(E_LOG,L_SCAN,"inbuf[%d]=0x%x\n", i,save_buf[i]);
        }
#endif
#if 0 
  if (rc == 0)
 {
	 for (i = 0; i < 32; i++)
        {       
                DPRINTF(E_LOG,L_SCAN,"outbuf_orig[%d]=0x%x\n", i, outbuf_orig[i]);
        }
}
#endif			
...
static unsigned char* get_utf8_text(const id3_ucs4_t* native_text) {
  ...
  /* (1) try utf8 -> cp932 */
	//debug_Aaron check whether the input is UTF-8 or not
  	rc = do_convert("EUC-KR", "UTF-8", in8, strlen(in8), iconv_buf, MAX_ICONV_BUF);
  	if (rc == ICONV_OK) 
  	{
    		utf8_text = in8;
    		free(iconv_buf);
  	...
  	{
    		/* (2) try cp932 -> utf8 */
    		rc = do_convert("UTF-8", "EUC-KR", in, strlen(in), iconv_buf, MAX_ICONV_BUF);
    		if (rc == ICONV_OK) 
		{
      			utf8_text = iconv_buf;
    		} 
...
#if 1 // EFM_PATCH
		char *ptr = strchr(utf8_text, ':');
		char *ptr1;
		if (ptr) // length is xx:yy.xx
		{
			*ptr = '\0';
			ptr++;
			pmp3->song_length = atoi(utf8_text)*60000;
			ptr1 = strchr(ptr, '.');
			if (ptr1)
			{
				*ptr1 = '\0';
				ptr1++;
				pmp3->song_length += atoi(ptr)*1000 + atoi(ptr1);
			}
		}
		else // length is ms.
#endif // EFM_PATCH
 ...
 */
#ifdef USE_EFMLOG
extern char *get_charset(char *locale);
extern int get_system_locale(char *locale);
#endif
...
void make_composite_tags(MP3FILE *song)
{
    ...
    if(!song->title) {
#if defined(USE_EFMLOG) && defined(USE_EUC_KR)
      char locale[32];

      get_system_locale(locale); // EFM, ysyoo

      if (!strcasecmp(get_charset(locale), "UTF-8")) {
	song->title = strdup(song->fname);
      } else {
	char* iconv_buf = (char*)calloc(MAX_ICONV_BUF, sizeof(char));
	if (iconv_buf) {
	  iconv_result rc = do_convert("UTF-8", get_charset(locale), 
