...
#ifdef EFM_PATCH
					con->response.content_length = atoll(value);
#else
...
static int cgi_demux_response(server *srv, handler_ctx *hctx) {
	...
	char proc[128], cmdline[1024];
	FILE *fp;
	sprintf(proc, "/proc/%d/cmdline", hctx->pid);
	fp = fopen(proc, "r");
	if (fp)
	{
		...
		if (!strncmp(cmdline, "download.cgi", 12) && !con->is_writable)
			return FDEVENT_HANDLED_NOT_FINISHED;
	}
#endif // EFM_PATCH || EFM_ROUTER_PATCH
	while(1) {
		int n;
...
#ifdef EFM_PATCH
		sprintf(buf,"%llu", con->request.content_length);
		//syslog(LOG_DEBUG,"--------> set CONTENT_LENGTH env -> %s", buf );
#else
