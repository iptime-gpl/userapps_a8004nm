...
#else
#error "Error: Make USE_LOCKS open in mod_webdav.c"
#endif
...
URIHANDLER_FUNC(mod_webdav_subrequest_handler) 
{
...
#ifdef EFM_WEBDAV
		//syslog(LOG_DEBUG,"--------> PROPFIND ... : %s", con->physical.path->ptr );

		if(stat(con->physical.path->ptr,&st) == -1)
		{
			con->http_status = 404;
		...
		case 1:
			if (NULL != (dir = opendir(con->physical.path->ptr))) {
				...
				while(NULL != (de = readdir(dir))) {
					...
					if (de->d_name[0] == '.' && de->d_name[1] == '\0') {
						/* don't append the . */
#ifdef EFM_PATCH // don't send current folder property.
						continue;
#endif // EFM_PATCH
					} else {
						buffer_append_string(d.path, de->d_name);
	...
	case HTTP_METHOD_MKCOL:
...
#ifdef EFM_WEBDAV
		if(!webdav_check_auth("rw",con->authed_user->ptr,con->uri.path->ptr))
		{
			webdav_efmlog(con,1,0,NULL);
			con->http_status = 403;
			con->keep_alive=0;
			return HANDLER_FINISHED;
		}
		webdav_efmlog(con,1,1,NULL);
#endif
		...
		//syslog(LOG_DEBUG,"MKDIR : %s", con->physical.path->ptr);
	...
	case HTTP_METHOD_DELETE:
		...
		//syslog(LOG_DEBUG,"HTTP_METHOD_DELETE .... %s", con->physical.path->ptr );
#ifdef EFM_WEBDAV
		if(!webdav_check_auth("rw",con->authed_user->ptr,con->uri.path->ptr))
		{
			webdav_efmlog(con,1,0,NULL);
		...
		if(!check_path_can_modified(con->uri.path->ptr))
		{
			webdav_efmlog(con,1,0,NULL);
			con->http_status = 403;
			return HANDLER_FINISHED;
	...
	case HTTP_METHOD_PUT: {
...
#ifdef EFM_WEBDAV
#if	0
		syslog(LOG_DEBUG,"------> HTTP_METHOD_PUT(%s): %s  , %s, length:%llu", 
				con->authed_user->ptr, 
				con->uri.path->ptr, 
				con->request.uri->ptr, 
		...
		if(!webdav_check_auth("rw",con->authed_user->ptr,con->uri.path->ptr))
		{
			//syslog(LOG_DEBUG,"409 -- Forbidden...");
			con->http_status = 409;
			webdav_efmlog(con,0,0,NULL);
			return HANDLER_FINISHED;
		}
#endif
		...
		close(fd);
#else
		if(!con->request.content_length && !con->request.chunk_encoding)
		{
			if (-1 == (fd = open(con->physical.path->ptr, O_WRONLY|O_TRUNC, WEBDAV_FILE_MODE))) {
				if (errno == ENOENT &&
				    -1 == (fd = open(con->physical.path->ptr, O_WRONLY|O_CREAT|O_TRUNC|O_EXCL, WEBDAV_FILE_MODE))) {
	...
	case HTTP_METHOD_COPY: {
...
#ifdef EFM_WEBDAV
		//syslog(LOG_DEBUG,"MOVE/COPY %d", con->request.http_method);
		if(!webdav_check_auth("rw",con->authed_user->ptr,con->uri.path->ptr))
		{
			webdav_efmlog(con,0,0,NULL);
			con->http_status = 403;
		...
		if(con->request.http_method == HTTP_METHOD_MOVE && !check_path_can_modified(con->uri.path->ptr))
		{
			webdav_efmlog(con,1,0,NULL);
			con->http_status = 403;
			return HANDLER_FINISHED;
		...
		if (p->physical.rel_path->ptr[0] == '/') {
...
#ifdef EFM_WEBDAV
			char *nptr;
			buffer_append_string(p->physical.path,"webdav/"); /* physical path is fixed to  /usr/webroot/webdav */
			if (con->authed_user->ptr)
			{
				if (strlen(con->authed_user->ptr) == 0)
					buffer_append_string(p->physical.path,"everybody");
				else
					buffer_append_string(p->physical.path,con->authed_user->ptr);
			}
			else
				buffer_append_string(p->physical.path,"everybody");
			nptr = strchr( &p->physical.rel_path->ptr[1],'/');
			if(!nptr) 
			{
				con->http_status = 403;
				return HANDLER_FINISHED;
			}
			buffer_append_string(p->physical.path, nptr);
#else
			...
			buffer_append_string(p->physical.path,"webdav/");
			if (con->authed_user->ptr)
			{
				if (strlen(con->authed_user->ptr) == 0)
					buffer_append_string(p->physical.path,"everybody");
				...
				buffer_append_string(p->physical.path,"everybody");
			nptr = strchr( &p->physical.rel_path->ptr[0],'/');
			if(!nptr) 
			{
				con->http_status = 403;
				return HANDLER_FINISHED;
		...
		//syslog(LOG_DEBUG,"------> p->phyiscal.path: %s", p->physical.path->ptr);
		//syslog(LOG_DEBUG,"------> p->uri.path: %s", p->uri.path->ptr);

#ifdef EFM_WEBDAV
		/* check destination folder auth */
		if(!webdav_check_auth("rw",con->authed_user->ptr,p->uri.path->ptr))
		{
			webdav_efmlog(con,0,0,NULL);
			con->http_status = 403;
			return HANDLER_FINISHED;
		...
		if (-1 == stat(con->physical.path->ptr, &st)) {
			...
			/* src is a directory */
#ifdef EFM_WEBDAV
			if (con->request.http_method == HTTP_METHOD_COPY) 
			{
#endif
			...
			if (con->request.http_method == HTTP_METHOD_MOVE) {
#ifdef EFM_WEBDAV
				//syslog(LOG_DEBUG,"run move : %s ---> %s ", con->physical.path->ptr, p->physical.path->ptr);
				system2("/bin/mv \"%s\" \"%s\"", con->physical.path->ptr, p->physical.path->ptr); 
				webdav_efmlog(con,1,1, p->physical.path->ptr);
#else
				b = buffer_init();
			...
			if (con->request.http_method == HTTP_METHOD_MOVE) {
				...
				//syslog(LOG_DEBUG,"MOVE : %s ---> %s ", con->physical.path->ptr, p->physical.path->ptr );
				if (0 == rename(con->physical.path->ptr, p->physical.path->ptr)) {

#ifdef EFM_WEBDAV
					webdav_efmlog(con,0,1,p->physical.path->ptr);
#endif
#ifdef USE_PROPPATCH
					sqlite3_stmt *stmt;
...
#ifdef EFM_WEBDAV
   	       //syslog(LOG_DEBUG,"------> HTTP_METHOD_PROPPATCH");
		if(!webdav_check_auth("rw",con->authed_user->ptr,con->uri.path->ptr))
		{
			con->http_status = 403;
			return HANDLER_FINISHED;
