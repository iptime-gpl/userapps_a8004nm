...
#else
static int proc_read_skip_nf_list(struct seq_file *seq, void *v)
{
	int i;
...
static int proc_open_skip_nf_list(struct inode *inode, struct file *file)
{
	return single_open(file, proc_read_skip_nf_list, NULL);
}
#endif
...
static int proc_write_skip_nf_list( struct file *file, const char __user *buffer, size_t count, loff_t *data )
{
        char buf[1024];
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
#else
static struct file_operations skipnf_fops = {
        .owner          = THIS_MODULE,
        .open           = proc_open_skip_nf_list,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .write          = proc_write_skip_nf_list,
...
#else
	if(!(proc_entry = proc_create(name, 0, 0, &skipnf_fops)))
