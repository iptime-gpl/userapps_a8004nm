		...
		struct sk_buff *skb)
{
	...
	if (skb->dev && skb->mark)
	{
		int INTERFACE_MARK_SUBID_BASE = 30;

		/* 2.4 Ghz MSSID QoS configured */
		if ((skb->mark > INTERFACE_MARK_SUBID_BASE) && (skb->mark < (INTERFACE_MARK_SUBID_BASE+5)))
			FOE_ALG(skb)=1;
		/* 5 Ghz MSSID QoS configured */
		if ((skb->mark > (INTERFACE_MARK_SUBID_BASE*2)) && (skb->mark < (INTERFACE_MARK_SUBID_BASE*2+5)))
			FOE_ALG(skb)=1;
		//printk("ifname: %s index : %d, base : %d, mark : %d\n", (skb->dev)?skb->dev->name:"null" , index, base, skb->mark);
	}
#endif
#endif
#endif
