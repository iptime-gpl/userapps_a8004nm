...
#define PATCH_FROM_MTK_SAMBA_READ_FOR_QDMA
...
#ifdef PATCH_FROM_MTK_SAMBA_READ_FOR_QDMA
...
//no hw van, no GE2, web UI used
	...
	if (len > 0) {
		...
		cpu_ptr->txd_info3.OWN_bit = 0;
		cpu_ptr->txd_info3.SWC_bit = 1;
		ei_local->skb_free[ctx_offset] = (cpu_ptr->txd_info3.LS_bit == 1)?skb:(struct sk_buff *)0xFFFFFFFF; 
